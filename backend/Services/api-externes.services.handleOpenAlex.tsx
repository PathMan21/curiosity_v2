    ...
    for (const item of subfieldIds) {
      ...
      if (cachedData && cachedData.articles && cachedData.articles.length > 0) {
        ...
        const recentUrl = `https://api.openalex.org/works?filter=topics.subfield.id:${subfieldId},is_oa:true,institutions.country_code:gb,language:en|publication_year:${currentYear - 1}-${currentYear}&per_page=${perPage}&page=${page}&sort=cited_by_count:desc`
          ...
          if (!response.ok) {
            console.warn(
              `⚠️ API OpenAlex non OK pour ${subfieldId}:`,
              response.status
            )
            continue
          }
        ...
        const filtered = allFetched.filter(
          (e) => e && e.topics && e.topics.some((t) => t.score >= 0.75)
        )
        try {
          ...
          console.warn(
            `⚠️ Impossible d'écrire dans Redis (${cacheKey}):`,
            err.message
          )
        }
...
function mapInterestsToSubfields(
  interestIds: string[]
): { subfield: string; journals: string[] }[] {
