...
async function setInCache(cacheKey, subfieldId, articles) {
  try {
    await redisClient.setEx(
      cacheKey,
      CACHE_TTL,
      JSON.stringify({ subfieldId, totalResults: articles.length, articles })
    )
...
async function fetchSubfieldFromAPI(subfieldId) {
  ...
  for (let page = 1; page <= MAX_PAGES; page++) {
    ...
    if (!response.ok) {
      console.warn(`⚠️ OpenAlex non OK (${subfieldId}) page ${page}:`, response.status)
      break
    }
...
async function resolveSubfields(subfieldItems) {
  ...
  await Promise.all(
    subfieldItems.map(async ({ subfield }) => {
      ...
      const articles = await fetchSubfieldFromAPI(subfield)
      allResults.push(...articles)
      await setInCache(cacheKey, subfield, articles)
    })
  )
...
async function handleOpenAlex(req, res) {
  try {
    ...
    const articles = shuffleArray(unique).slice(0, MAX_FINAL_RESULTS).map(formatWork)
